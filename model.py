...
class DXVAE(nn.Module):
    ...
    def _propagate(self, G, v, H_in=None, encode=False):
        ...
        # get X and X_loop
        X = torch.stack([g.ndata['params'][v] for g in G])  # (batch, size_params)
    ...
    def decode(self, z):
        ...
        for g in G:
            g.add_nodes(1, {'params': self.zero_params.unsqueeze(0),  # add X0
                            'hidden': self.zero_hidden.unsqueeze(0)})  # empty H0
        ...
        for vi in range(1, self.n_nodes):
            ...
            Ei_self = self.h_to_edge_self(Hi) > 0.5  # (batch, 1) boolean
            for idx, g in enumerate(G):
                if Ei_self[idx]:
                    g.add_edges(vi, vi)
            Hi = self._propagate(G, vi)
    ...
    def loss(self, q_dist, G_true, w1=10, w2=0.2, w3=1, w4=1):
        ...
        param_true = torch.stack(
            [g_true.ndata['params']
             for g_true in G_true])  # (batch, n_nodes, size_params)
        adj_true = torch.stack(
            [g_true.adj().to_dense()
             for g_true in G_true])  # (batch, n_nodes, n_nodes)
        ...
        for vi in range(1, self.n_nodes):
            ...
            Ei_self_true = adj_true[:, vi, vi].unsqueeze(1)  # (batch, 1)
            for idx, g in enumerate(G):
                if Ei_self_true[idx]:
                    g.add_edges(vi, vi)
            Hi = self._propagate(G, vi)
            ...
            loss_edges += F.binary_cross_entropy(
                Ei_self, Ei_self_true, reduction='sum')

            # consider in/out-edge probabilities
            Ei = []
            adj_in_true = adj_true[:, :vi, vi].unsqueeze(2)  # (batch, vi, 1)
            adj_out_true = adj_true[:, vi, :vi].unsqueeze(2)  # (batch, vi, 1)
            ...
            Ei.reverse()
            Ei = torch.cat(Ei, 1)  # (batch, vi, 2) in/out prob
            # compute out/in-edge losses
            loss_edges += F.binary_cross_entropy(Ei, Ei_true, reduction='sum')
